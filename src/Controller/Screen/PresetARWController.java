 ...
 * @author user
 */
public class PresetARWController implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		try {
			storedResponse = new Response(FieldService.readFieldsFromFile(new File(Main.PRESET_ARW_FIELDS_PATH), true));
			...
			AlertController.showAlert("Error", "Cannot open ARW presets",
					"We cannot open the ARW presets file. Does it exist?", Alert.AlertType.ERROR);
		}
	}
	...
	public void editAction() {
		...
		// Reset the title
		titleText.setText("Edit ARW Form");
	...
	public void saveAction() {
		...
		if (file != null) {
			fileChooser.setTitle("Choose where to save the responses file");
			fileChooser.getExtensionFilters().clear();
			extFilter = new FileChooser.ExtensionFilter("Excel file (*.xlsx)", "*.xlsx");
			fileChooser.getExtensionFilters().add(extFilter);
			...
			if (outputFile != null) {
				try {
					...
					ResponseService.createForm(file, outputFile.getAbsolutePath(), false);

					// 2) The original ARW fields + the custom fields added by the user (if any)
					ResponseService.createForm(file, outputFile.getAbsolutePath(), true);
	...
	public void doneAction() {
		// Reset the title
		titleText.setText("Preview ARW Form");
	...
	public void addQuestionAction() {
		...
		result.ifPresent(question -> {
			...
			if (question.isEmpty() || question.contains(">") || question.contains(",")) {
				AlertController.showAlert("Error", "Invalid label", "You did not enter a valid label.",
						Alert.AlertType.ERROR);
			} else {
				// Add a new field
